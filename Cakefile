...
build = (callback) ->
  ...
  options = ['-c','-b', '-j', 'public/client/js/application.js',
             'src/client/js']
  cmd = which.sync 'coffee'
  coffee = spawn cmd, options
  coffee.stdout.pipe process.stdout
...
task 'dev', 'start dev env', ->
  # watch_coffee
  options = ['-w', '-b', '-c', '-o', '.app', 'src/server']
  cmd = which.sync 'coffee'  
  coffee = spawn cmd, options
  coffee.stdout.pipe process.stdout
  coffee.stderr.pipe process.stderr

  options = ['-c', '-b', '-w', '-j', 'public/client/js/application.js',
             'src/client/js']
  coffee = spawn cmd, options
  coffee.stdout.pipe process.stdout
  coffee.stderr.pipe process.stderr
